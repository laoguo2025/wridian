...
- 正文修改链路参考 `obsidian-copilot` 的 `replaceGuard.ts` 和 `claudian` 的当前编辑器上下文：用户在对话里要求修改正文时，默认针对当前打开文件；选区触发的修改优先验证选区 start/end/text 快照，无范围快照时只有 target 在当前正文中唯一命中且不与其他修改范围重叠，才在正文编辑区显示待确认内联 diff。找不到、重复出现或重叠的建议会保持待确认状态，并提示用户确认是否已打开正确文件，禁止默认改第一处。普通聊天、建议、解释和比较不应自动改正文。
