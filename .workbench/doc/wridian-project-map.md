...
- 对话请求默认携带作品库和知识库的相对文件树，不向模型暴露本机绝对路径。模型若要操作文件树，必须返回 `fileOperations`，后端只按当前作品库/知识库内的相对路径执行 `writeFile/createFolder/rename/trash`，并继续复用本地安全校验；兼容/畸形输出中可恢复的 `fileOperations` 仍按结构化操作执行，不得把工具载荷当聊天正文显示。作品库 `writeFile` 必须写入用户已绑定的本地作品库；未绑定或绑定目录失效时不得回落到默认 `works` 根。用户正在打开作品文件时，模型只给文件名或冗余 `works/文件名` 的新建作品文件默认写到当前文件所在文件夹。`writeFile` 只允许新建不存在的 md/txt/docx 文件并写入初始内容，对已有文件内容执行新增、修改、删除时必须返回 `edits` 并由正文内联 diff 确认。用户明确要求新建、写入、保存、重命名、删除或创建目录这类文件树变更但模型没有返回 `fileOperations` 时，后端必须自动补救请求一次可执行 `fileOperations`；若补救仍失败但模型回复是可作为新文档内容的实质正文，前端会借鉴 Claudian 的工具流形态合成本地 `writeFile` 工具操作并调用后端受限执行命令，结果仍作为独立工具块展示；若模型只是声称已新建/已写入而无正文，则拦截为未执行提示，不得展示假成功。前端收到成功文件树操作后刷新文件树；聊天气泡保留模型正常回复并按 Markdown 预览渲染标题、列表、表格、代码和加粗等基础格式，表格在气泡内自适应并可横向滚动；文件树操作结果作为独立工具结果块展示，不再覆盖或拼接进普通回复正文。
...
- 模型设置弹窗入口为 `src/settings/ModelSettingsDialog.tsx`，尺寸与知识图谱一致，采用上下布局：“已连接服务”在上，“添加服务”在下。添加服务按“授权登录 / 国内服务 / 第三方API”分组，同页三列展示；Aliyun Bailian Coding Plan 与 Aliyun Bailian Token Plan 是两个独立 provider；所有 provider 卡片为“名称/描述 + 右侧连接或断开按钮”，不显示头像图标、底部接入类型标签或已连接详情表；二级连接弹窗不提供“查看接入详情”展开区或外链文档/控制台按钮；已配置 provider 从添加区消失，取消配置后恢复。
...
- 正文修改链路参考 `obsidian-copilot` 的 `replaceGuard.ts` 和 `claudian` 的当前编辑器上下文：用户在对话里要求修改正文时，默认针对当前打开文件；选区触发的修改优先验证选区 start/end/text 快照。明确字面替换指令（如“把 A 都改成 B”）必须先走本地确定性替换，给每个命中位置生成带 sourceRange 的待确认内联 diff，禁止让模型引申修改其他词。模型建议类修改只有 target 在当前正文中唯一命中且不与其他修改范围重叠，才在正文编辑区显示待确认内联 diff。找不到、重复出现或重叠的建议会保持待确认状态，并提示模型建议未安全定位，禁止默认改第一处。普通聊天、建议、解释和比较不应自动改正文。
...
2. 实现最小对话请求。
   ...
   - 第一版只展示回复，不自动改正文。
3. 做回复到正文的安全操作。
   - 支持用户选中正文片段并添加到输入框；正文选区形成后应自动浮出“添加到对话”，不要求用户再点击正文触发。
   - 对话回复底部支持重试、复制；用户消息底部支持修改、复制。
   - Wridian 只在用户明确要求改正文时进入正文修改链路；能安全定位的修改先在正文内联 diff 展示，红色为删除、绿色为新增，文件顶部提供靠近排列的全部确认和全部取消，每处修改提供确认和取消。
   - 明确字面替换不调用模型生成 edits；本地扫描当前正文或选区，每个命中都用 sourceRange 锁定，确认后只替换用户指定的字面词。
   - 无法安全定位的正文修改保留待确认状态并提示重新定位，不发到对话区冒充正文修改。
   - 不做自动全文覆盖。
   - 确认模型修改后，文件标题右侧提供一次性“撤销修改”：只回退最近一次写入前内容；撤销一次、手动编辑、切换文件或后续新修改都会让旧回退状态失效。
   - 当前已完成：正文区使用纯文本编辑器承载 inline diff，确认后继续走已有自动保存链路；inline diff 只是编辑器内的待确认建议，不是单独审阅模式。
4. 完成记忆树注入闭环。
