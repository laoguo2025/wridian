...
- 当前对齐的 `obsidian-copilot` 源码基线：
  ...
  - `ChatManager.ts`：Wridian 已引入本地前端版 `src/chat/chatManager.ts`，负责消息列表、pending/error、发送共创请求、追加助手回复和生成待确认正文修改；`src/chat/cocreationClient.ts` 承载 `wridian_cocreate` 的 Tauri 请求/响应类型和参数组装。
