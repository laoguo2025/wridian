...
- 正文修改链路参考 `obsidian-copilot` 的 `replaceGuard.ts` 和 `claudian` 的当前编辑器上下文：用户在对话里要求修改正文时，默认修改当前打开文件；选区触发的修改优先验证选区 start/end/text 快照，无范围快照时只有 target 在当前正文中唯一命中且不与其他修改范围重叠才自动写入正文；找不到、重复出现或重叠的建议会保持待确认状态，并提示用户确认是否已打开正确文件，禁止默认改第一处。普通聊天、建议、解释和比较不应自动改正文。
...
3. 做回复到正文的安全操作。
   ...
   - Wridian 只在用户明确要求改正文时落地正文修改；能安全定位的修改直接写入正文、在正文内保留已应用修改高亮，并保留一次性撤销。
   - 无法安全定位的正文修改仍以正文内联 diff 展示，红色为删除、绿色为新增；文件顶部提供靠近排列的全部确认和全部取消，每处修改提供确认和取消。
   - 不做自动全文覆盖。
   - 自动写入或确认模型修改后，文件标题右侧提供一次性“撤销修改”：只回退最近一次写入前内容；撤销一次、手动编辑、切换文件或后续新修改都会让旧回退状态失效。
   - 当前已完成：正文区使用纯文本编辑器承载 inline diff，确认或自动写入后继续走已有自动保存链路；inline diff 只是编辑器内的待确认建议，不是单独审阅模式。
4. 完成记忆树注入闭环。
