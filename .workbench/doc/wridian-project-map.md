...
- 对话请求默认携带作品库和知识库的相对文件树，不向模型暴露本机绝对路径。模型若要操作文件树，必须返回 `fileOperations`，后端只按当前作品库/知识库内的相对路径执行 `writeFile/createFolder/rename/trash`，并继续复用本地安全校验；`writeFile` 只允许新建不存在的 md/txt/docx 文件并写入初始内容，对已有文件内容执行新增、修改、删除时必须返回 `edits` 并由正文内联 diff 确认。前端收到成功文件树操作后刷新文件树。
