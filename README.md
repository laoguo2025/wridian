...
- 知识图谱补强为更像写作知识网络：支持关系邻域预览，悬浮知识卡时能看到出链、反链和关系分组。
- 图谱治理信息更清晰：节点类型、来源链路、重复概念、孤岛知识、作品引用和知识卡反链更容易被看见。
- 增加知识 hot cache：对话会轻量参考最近显式使用过的知识卡，减少重复手动找卡。
- Relevant Notes 升级为分段召回，能返回更贴近当前稿件和提问的命中片段。
- 助手回复可手动“存为卡”，写入知识库对话沉淀区，作为待核查草稿卡，不绕过正式知识卡质量流程。
- 增加本地写入锁，覆盖稿件、知识文件、聊天归档、记忆树、模型账号和工作区配置等写入链路，降低多处同时写入时的冲突风险。
...
- [release/Wridian-0.0.4-x64-setup.exe](release/Wridian-0.0.4-x64-setup.exe)

也可以从 GitHub Release 下载：

- [Wridian 0.0.4](https://github.com/laoguo2025/wridian/releases/tag/v0.0.4)
