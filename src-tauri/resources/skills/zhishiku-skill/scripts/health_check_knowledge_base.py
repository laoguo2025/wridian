...
def write_health_report(root: Path, issues: list[str], review_marks: list[str]) -> Path:
    ...
    report_path = report_dir / f"知识库体检-{date.today().isoformat()}.md"
    highest = issues[0] if issues else "无阻断问题"
    lines = [
        f"# 知识库体检 {date.today().isoformat()}",
        "",
        "## 总览",
        f"- 问题总数：{len(issues)}",
        f"- 最高风险：{highest}",
...
def main() -> int:
    ...
    parser.add_argument("--report", action="store_true", help="write 00知识库治理/知识库体检-YYYY-MM-DD.md")
